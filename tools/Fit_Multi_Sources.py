...
# ---------------------------------------------------------------------------
# Model M — Mixture of multiple spatial sources (EM hard clustering)
# ---------------------------------------------------------------------------

def _kmeans_init_positions(X: np.ndarray, k: int, seed: int = 42, n_iter: int = 15) -> np.ndarray:
    """
    Lightweight K-means (no sklearn dependency) used only for initialising mixture clusters.
    ...
    seed : RNG seed
    n_iter : iterations for the kmeans init
    ...
    n = X.shape[0]
    # Pick k distinct initial centers
...
def _log10_pred_for_A_single_point(p: SystemPoint, fit_result: Dict[str, Any]) -> float:
    """Predict log10(r) for a single SystemPoint given a Model A fit result."""
    S = np.array(fit_result["source"], dtype=np.float64)
    alpha = float(fit_result["alpha"])
    logK = float(fit_result["K_log10"])
    d = float(np.linalg.norm(p.pos - S))
    d = max(d, 1.0)
    return logK - alpha * math.log10(d)
...
def fit_model_M(
    ...
    """
    Model M: Mixture of K isotropic point sources (hard EM).

    This is intentionally simple and robust:
      - Initialise clusters using K-means on positions
      - Alternate:
          * Fit Model A separately on each cluster
          * Reassign each system to the source that minimises |residual| in log10(r)
      - Stop when assignments stabilise or after max_iter

    Output contains:
      - per-source Model A fits
      - per-system cluster assignment
      - cluster sizes
    """
    n = len(points)
    if k < 2:
        return {"error": f"Model M requires k>=2, got {k}"}
    if n < max(4, k * 3):
        return {"error": f"Not enough systems for k={k} (need at least {max(4, k * 3)}), got {n}"}

    X = np.stack([p.pos for p in points])
    labels = _kmeans_init_positions(X, k=k, seed=seed)

    def build_clusters(lbls: np.ndarray) -> List[List[SystemPoint]]:
        clusters = [[] for _ in range(k)]
        for i, lab in enumerate(lbls.tolist()):
            clusters[int(lab)].append(points[i])
        return clusters

    clusters = build_clusters(labels)
    prev_assignments: Optional[Dict[str, int]] = None

    for _ in range(max_iter):
        # Fit each cluster (Model A). If a cluster is too small, mark as None.
        fits: List[Optional[Dict[str, Any]]] = []
        for c in clusters:
            if len(c) < 4:
                fits.append(None)
            else:
                r = fit_model_A(c, alpha_bounds=alpha_bounds, source_reg_weight=source_reg_weight)
                fits.append(None if ("error" in r) else r)

        # Reassign each point to the best-fitting source
        ...
        for i, p in enumerate(points):
            ...
            for j, fit in enumerate(fits):
                if fit is None:
                    continue
                pred = _log10_pred_for_A_single_point(p, fit)
                err = abs(p.log10_r - pred)
                if err < best_err:
                    best_err = err
                    best_k = j
            if best_k is None:
                # All fits missing (e.g. too-small clusters): fall back to original label
                best_k = int(labels[i])
            assignments[p.name] = int(best_k)
            new_labels[i] = int(best_k)
    ...
    # Final per-cluster Model A fits
    sources: List[Dict[str, Any]] = []
    for c in clusters:
        if len(c) < 4:
            sources.append({"error": f"cluster too small (n={len(c)})"})
        else:
            sources.append(fit_model_A(c, alpha_bounds=alpha_bounds, source_reg_weight=source_reg_weight))

    # Build per-system residuals vs assigned source (handy for debugging)
    per_system = []
    for p in points:
        cid = int(assignments.get(p.name, 0))
        fit = sources[cid] if 0 <= cid < len(sources) else None
        if isinstance(fit, dict) and "error" not in fit:
            ...
            per_system.append({
                ...
                "cluster": cid,
                "snr_db_obs": p.snr_db,
                "snr_db_pred": 10.0 * pred,
                "residual_log10r": float(p.log10_r - pred),
            })
        else:
            per_system.append({"name": p.name, "cluster": cid, "note": "no valid fit for cluster"})

    return {
        "model": "M",
        "converged": True,
        "n_sources": int(k),
        "mixture_max_iter": int(max_iter),
        ...
        "interpretation": "Hard-EM mixture of isotropic sources (Model A per cluster). Use residuals and cluster membership to assess multi-regime structure.",
    }
...
def bootstrap_fit(
    ...
    def fit_fn(subset: List[SystemPoint]) -> Dict[str, Any]:
        ...
        if model == "M":
            # Bootstrap for mixtures is more expensive; still supported.
            return fit_model_M(subset, k=2, max_iter=20, alpha_bounds=alpha_bounds, source_reg_weight=source_reg_weight, seed=rng_seed)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", choices=["A", "B", "C", "M", "all"], default="all", help="Which model(s) to run")
    ...
    p.add_argument("--k", type=int, default=2, help="Number of mixture sources (Model M)")
    p.add_argument("--mixture-iter", type=int, default=20, help="Mixture EM iterations (Model M)")
    return p.parse_args()
...
def main() -> int:
    ...
    models = ["A", "B", "C", "M"] if args.model == "all" else [args.model]
    ...
    for m in models:
        ...
        if m == "A":
            ...
            result = fit_model_M(points, k=int(args.k), max_iter=int(args.mixture_iter), alpha_bounds=alpha_bounds, source_reg_weight=float(args.source_reg), seed=int(args.seed))
        else:
        ...
        if result.get("skipped"):
            ...
            continue
        if m == "M":
            k = result.get("n_sources")
            sizes = result.get("cluster_sizes")
            print(f"  Mixture: k={k}  cluster_sizes={sizes}")
            for j, sub in enumerate(result.get("sources", [])):
                if not isinstance(sub, dict) or "error" in sub:
                    err = sub.get("error") if isinstance(sub, dict) else "n/a"
                    print(f"    [cluster {j}] ERROR: {err}")
                    continue
                src = sub.get("source") or [None, None, None]
                alpha = sub.get("alpha")
                rmse = (sub.get("residuals") or {}).get("rmse_log10r")
                if all(isinstance(v, (int, float)) for v in src):
                    print(f"    [cluster {j}] Source : [{src[0]:.0f}, {src[1]:.0f}, {src[2]:.0f}]")
                else:
                    print(f"    [cluster {j}] Source : {src}")
